...
function PlayingCard({
  ...
  const rotation = offset * (isMobile ? 4 : 5);
  const translateX = offset * (isMobile ? 22 : 32);
  const cardW = isMobile ? 65 : 85;
  const cardH = isMobile ? 95 : 120;
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center px-1">
          <span
            ...
            style={{ fontSize: skill.length > 10 ? (isMobile ? "7px" : "8px") : (isMobile ? "8px" : "9.5px"), color: "#1a1a1a", fontFamily: "var(--font-inter), sans-serif" }}
          >
            {skill}
          </span>
        </div>
...
function CategoryHand({ cat }: { cat: typeof CATEGORIES[number] }) {
  ...
  const isMobile = typeof window !== "undefined" && window.innerWidth < 768;
  const containerH = isMobile ? 130 : 170;
  const containerW = isMobile
    ? Math.max(cat.skills.length * 26 + 65, 180)
    : Math.max(cat.skills.length * 36 + 85, 220);
